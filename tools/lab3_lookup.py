...
class Lookup(object):
    ...
    def getJSONRequest(self, params, attempts=3):
        ...
        try:
            ...
            print("Lookup '{}' failed, error: {}. Attempts: {}".format(
                url, e, str(attempts)))
            time.sleep(3)  #to avoid limit of calls, sleep 3s
            attempts -= 1
            if attempts > 0:
...
class DBpediaLookup(Lookup):
    ...
    def __extractKGEntities(self, json, filter=''):
        ...
        for element in json['docs']:
            ...
            kg_entity = KGEntity(uri, label, description, types,
                                 self.getKGName())
    ...
    def getKGEntities(self, query, limit, filter='') -> list[KGEntity]:
        ...
        return self.__extractKGEntities(json,
                                        filter)  #Optionally filter by URI
...
class WikidataAPI(Lookup):
    ...
    def __extractKGEntities(self, json, filter=''):
        ...
        for element in json['search']:
            ...
            kg_entity = KGEntity(element['concepturi'], element['label'],
                                 description, types, self.getKGName())
...
class GoogleKGLookup(Lookup):
    ...
    def __extractKGEntities(self, json, filter=''):
        ...
        for element in json['itemListElement']:
            ...
            kg_entity = KGEntity(element['result']['@id'],
                                 element['result']['name'], description, types,
                                 self.getKGName())
