...
class SPARQLEndpoint(object):
    ...
    def getQueryResultsArityOne(self, query, filter_uri=True):
        ...
        for result in results["results"]["bindings"]:
            ...
            if not filter_uri or uri_value.startswith(
                    URI_KG.dbpedia_uri) or uri_value.startswith(
                        URI_KG.wikidata_uri) or uri_value.startswith(
                            URI_KG.schema_uri) or uri_value.startswith(
                                URI_KG.dbpedia_uri_resource
                            ) or uri_value.startswith(
    ...
    def getQueryResultsArityTwo(self,
                                query,
                                filter_outA=True,
                                filter_outB=True):
        ...
        for result in results["results"]["bindings"]:
            ...
            if not filter_outA or outA_value.startswith(
                    URI_KG.dbpedia_uri) or outA_value.startswith(
                        URI_KG.wikidata_uri) or outA_value.startswith(
                            URI_KG.schema_uri) or outA_value.startswith(
                                URI_KG.dbpedia_uri_resource
                            ) or outA_value.startswith(
                                URI_KG.dbpedia_uri_property):

                if not filter_outB or outB_value.startswith(
                        URI_KG.dbpedia_uri) or outB_value.startswith(
                            URI_KG.wikidata_uri) or outB_value.startswith(
                                URI_KG.schema_uri) or outB_value.startswith(
                                    URI_KG.dbpedia_uri_resource
                                ) or outB_value.startswith(
                                    URI_KG.dbpedia_uri_property):
    ...
    def createSPARQLQueryPredicatesForSubject(self, subject, limit=1000):
        return "SELECT DISTINCT ?uri WHERE { <" + subject + "> ?uri [] . } limit " + str(
            limit)

    def createSPARQLQueryPredicatesForObject(self, obj, limit=1000):
        return "SELECT DISTINCT ?uri WHERE { [] ?uri <" + obj + "> . } limit " + str(
            limit)

    def createSPARQLQueryDomainTypesOfPredicatesForSubject(
            self, subject, limit=1000):
        return "SELECT DISTINCT ?uri WHERE { <" + subject + "> ?p [] . ?p rdfs:domain ?uri . } limit " + str(
            limit)

    def createSPARQLQueryRangeTypesOfPredicatesForObject(
            self, obj, limit=1000):
        return "SELECT DISTINCT ?uri WHERE { [] ?p <" + obj + "> . ?p rdfs:range ?uri . } limit " + str(
            limit)

    def createSPARQLQuerySomeValuesForPredicate(self, predicate, limit=100):
        return "SELECT DISTINCT ?uri WHERE { ?s <" + predicate + "> ?uri . } limit " + str(
            limit)
    ...
    def createSPARQLQueryDomainTypesCountOfPredicatesForSubject(
            self, subject, limit=3):
    ...
    def createSPARQLQueryRangeTypesCountOfPredicatesForObject(
            self, obj, limit=3):
...
class DBpediaEndpoint(SPARQLEndpoint):
    ...
    def createSPARQLEntitiesLabelsForClass(self,
                                           class_uri,
                                           offset=0,
                                           limit=1000):
...
class WikidataEndpoint(SPARQLEndpoint):
    ...
    def createSPARQLEntitiesForClass(self, class_uri, offset=0, limit=1000):
        ...
        return "SELECT DISTINCT ?uri WHERE { ?uri <http://www.wikidata.org/prop/direct/P31> <" + class_uri + "> . } ORDER BY RAND() OFFSET " + str(
            offset) + " limit " + str(limit)
